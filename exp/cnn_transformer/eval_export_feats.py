...
if not os.path.exists(os.path.join(out_dir, 'imgs')):
    os.mkdir(os.path.join(out_dir, 'imgs'))
...
def eval():
    ...
    with tf.device('/gpu:'+str(gpu_id)):
        ...
        pred, last_layer = model_type.get_model(input_ph, is_training=is_training_ph, \
    ...
    feats = np.zeros((0, feat_len), dtype=np.float32)
    filelist = []
    for i in range(num_test_file):
        cur_test_filename = test_file_list[i]

        ori_data, cur_labels = load_h5(cur_test_filename)
        cur_data = np.array(ori_data, dtype=np.float32)
        ...
        for j in range(num_batch):
            ...
            pred_val, feats_val = sess.run([pred, last_layer], feed_dict=feed_dict)

            feats = np.concatenate([feats, feats_val], axis=0)
            
            label_pred = np.argmax(pred_val, 1)

            for k in range(begidx, endidx):
                info = 'pred_'+catid2catname[label_pred[k-begidx]]+'_gt_'+catid2catname[cur_labels[k]]
                Image.fromarray(np.uint8(ori_data[k])).save(os.path.join(out_dir, 'imgs', str(i)+'_'+str(k)+'_'+info+'.png'))
                filelist.append(os.path.join(str(i)+'_'+str(k)+'_'+info+'.png'))

            total_correct += np.sum(label_pred == cur_labels[begidx: endidx])
            total_seen += batch_size
    
    json.dump(filelist, open(os.path.join(out_dir, 'photos.json'), 'w'))
