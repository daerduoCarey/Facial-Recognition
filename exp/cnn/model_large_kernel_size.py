...
def get_model(input_images, is_training, cat_num, batch_size, weight_decay, bn_decay):
    ...
    net = tf_util.conv2d(input_data, 128, [11, 11], stride=[1, 1], padding='SAME', scope='conv1', \
    ...
    net = tf_util.conv2d(net, 256, [11, 11], stride=[1, 1], padding='SAME', scope='conv2', \
            weight_decay=weight_decay, bn_decay=bn_decay, bn=True, is_training=is_training)
    net = tf_util.max_pool2d(net, [2, 2], stride=[2, 2], scope='mp2', padding='SAME')
    print net

    net = tf_util.conv2d(net, 512, [7, 7], stride=[1, 1], padding='SAME', scope='conv3', \
            weight_decay=weight_decay, bn_decay=bn_decay, bn=True, is_training=is_training)
    net = tf_util.max_pool2d(net, [2, 2], stride=[2, 2], scope='mp3', padding='SAME')
    print net

    net = tf_util.conv2d(net, 512, [7, 7], stride=[1, 1], padding='SAME', scope='conv4', \
            weight_decay=weight_decay, bn_decay=bn_decay, bn=True, is_training=is_training)
