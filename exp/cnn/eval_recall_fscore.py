...
def eval():
    ...
    per_cat_recall_seen = np.zeros((total_cat_num), dtype=np.int32)
    per_cat_recall_correct = np.zeros((total_cat_num), dtype=np.int32)
    per_cat_precision_seen = np.zeros((total_cat_num), dtype=np.int32)
    per_cat_precision_correct = np.zeros((total_cat_num), dtype=np.int32)
    for i in range(num_test_file):
        ...
        for j in range(num_batch):
            ...
            for k in range(begidx, endidx):
                ...
                per_cat_recall_seen[cur_gt_label] += 1
                per_cat_recall_correct[cur_gt_label] += np.int32(cur_gt_label == cur_pred_label)
                per_cat_precision_seen[cur_pred_label] += 1
                per_cat_precision_correct[cur_pred_label] += np.int32(cur_gt_label == cur_pred_label)

    for i in range(total_cat_num):
        per_cat_acc = per_cat_recall_correct[i] * 1.0 / per_cat_recall_seen[i]
        per_cat_pre = per_cat_precision_correct[i] * 1.0 / per_cat_precision_seen[i]
        per_cat_fscore = 2 * per_cat_acc * per_cat_pre / (per_cat_acc + per_cat_pre)
        printout(flog, 'Per-cat Accuracy (Recall) %s: %f' % (catid2catname[i], per_cat_acc))
        printout(flog, 'Per-cat Precision %s: %f' % (catid2catname[i], per_cat_pre))
        printout(flog, 'Per-cat F-score %s: %f\n' % (catid2catname[i], per_cat_fscore))
