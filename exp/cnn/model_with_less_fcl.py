...
def get_model(input_images, is_training, cat_num, batch_size, weight_decay, bn_decay):
    ...
    net = tf_util.conv2d(input_data, 128, [5, 5], stride=[1, 1], padding='VALID', scope='conv1', \
            weight_decay=weight_decay, bn_decay=bn_decay, bn=True, is_training=is_training)

    net = tf_util.conv2d(net, 256, [5, 5], stride=[1, 1], padding='VALID', scope='conv2', \
            weight_decay=weight_decay, bn_decay=bn_decay, bn=True, is_training=is_training)

    net = tf_util.conv2d(net, 256, [5, 5], stride=[1, 1], padding='VALID', scope='conv3', \
            weight_decay=weight_decay, bn_decay=bn_decay, bn=True, is_training=is_training)

    net = tf_util.conv2d(net, 256, [5, 5], stride=[1, 1], padding='VALID', scope='conv4', \
            weight_decay=weight_decay, bn_decay=bn_decay, bn=True, is_training=is_training)

    net = tf_util.conv2d(net, 512, [5, 5], stride=[1, 1], padding='VALID', scope='conv5', \
            weight_decay=weight_decay, bn_decay=bn_decay, bn=True, is_training=is_training)
    net = tf_util.max_pool2d(net, [2, 2], stride=[2, 2], scope='mp3', padding='VALID')

    net = tf_util.conv2d(net, 512, [5, 5], stride=[1, 1], padding='VALID', scope='conv6', \
            weight_decay=weight_decay, bn_decay=bn_decay, bn=True, is_training=is_training)
    ...
    net = tf_util.dropout(net, is_training=is_training, keep_prob=0.8, scope='dp1')

    net = tf_util.fully_connected(net, 128, bn=True, is_training=is_training, \
            weight_decay=weight_decay, bn_decay=bn_decay, scope='fc2')
    net = tf_util.dropout(net, is_training=is_training, keep_prob=0.8, scope='dp2')
