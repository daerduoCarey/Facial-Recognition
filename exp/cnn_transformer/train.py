...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(gpu_id)):
            ...
            pred, theta, transformed_input = model_type.get_model(input_ph, is_training=is_training_ph, \
        ...
        def train_one_epoch(train_file_idx, epoch):
            ...
            for i in range(num_train_file):
                ...
                for j in range(num_batch):
                    ...
                    _, loss_val, transformed_val, theta_val, pred_val, lrv = sess.run([train_op, total_loss_op, \
                            transformed_input, theta, pred, learning_rate], feed_dict=feed_dict)
                    label_pred = np.argmax(pred_val, 1)
                    
                    if i == 0 and j == 0:
                        print 'Learning Rate: ', lrv
                    
                    printout(flog, str(theta_val[0, :]))
                    ...
                    if verbose and epoch % 1 == 0:
                        ...
                        for shape_idx in range(begidx, endidx):
                            ...
                            if not gt_label == pred_label:
                                ...
                                tran_image = transformed_val[shape_idx-begidx, :, :, 0] / image_scale + 128
                                ...
                                Image.fromarray(np.uint8(tran_image)).save(os.path.join(log_epoch_dir, str(train_file_idx[i])+'_'+str(shape_idx)+'_transformed'+'.jpg'))
        ...
        def eval_one_epoch(epoch):
            ...
            for i in range(num_test_file):
                ...
                for j in range(num_batch):
                    ...
                    loss_val, pred_val, transformed_val, theta_val = sess.run([total_loss_op, pred, transformed_input, theta], \
                            feed_dict=feed_dict)
                    label_pred = np.argmax(pred_val, 1)
                    ...
                    for shape_idx in range(begidx, endidx):
                        ...
                        if verbose and epoch % 1 == 0:
                            ...
                            if not gt_label == pred_label:
                                ...
                                Image.fromarray(np.uint8(tran_image)).save(os.path.join(log_epoch_dir, str(shape_idx)+'_transformed'+'.jpg'))
